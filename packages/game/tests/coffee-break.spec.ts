...
test.describe('Coffee Break Card Test', () => {
  test('should correctly play Coffee Break card and verify game state', async ({ page }) => {
    ...
    while (!foundCoffeeBreak && roundCount < maxRounds) {
      ...
      for (let i = 0; i < cardCount; i++) {
        ...
        const cardTitle = await card.locator('[class*="cardTitle"]').textContent();
        ...
        if (cardTitle === 'Coffee Break') {
          ...
          // Get initial state before playing the card
          ...
          // Handle any coin flip if present (though Coffee Break shouldn't have one)
          ...
          // Wait for any card drawing animations to complete
          ...
          console.log('✅ Coffee Break card test passed!');
      ...
      if (!foundCoffeeBreak) {
        ...
        try {
          await page.waitForSelector(':has-text("drawing cards")', { timeout: 5000 });
  ...
  async function getGameStats(page: any) {
    ...
    const progressValue = await page.locator('[data-type="progress"] [class*="resourceValue"]').first().textContent();
    const bugsValue = await page.locator('[data-type="bugs"] [class*="resourceValue"]').first().textContent();
    const techDebtValue = await page.locator('[data-type="technical-debt"] [class*="resourceValue"]').first().textContent();
    const productivityValue = await page.locator('[data-type="productivity"] [class*="resourceValue"]').first().textContent();
    
    // Get stats from GameInfo
    const cardsInHand = await page.locator('[class*="cards"] > div').count();
    const cardsPlayedStat = statsContainer.locator('[class*="stat"]').filter({ hasText: 'Cards Played' });
    const cardsRemainingstat = statsContainer.locator('[class*="stat"]').filter({ hasText: 'Cards Remaining' });
    
    const cardsPlayed = await cardsPlayedStat.locator('[class*="statValue"]').textContent();
    const cardsRemaining = await cardsRemainingstat.locator('[class*="statValue"]').textContent();

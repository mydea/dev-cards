...
test.describe('Dev-Cards Game Integration', () => {
  test('should play a complete game from start to finish', async ({ page }) => {
    ...
    while (!gameEnded && roundCount < maxRounds) {
      ...
      const gameOverVisible = await page.getByText('Victory!').isVisible().catch(() => false) ||
                             await page.getByText('Game Over').isVisible().catch(() => false);
      ...
      while (true) {
        ...
        for (let i = 0; i < cardCount; i++) {
          ...
          const isPlayable = await cardElement.evaluate((el) => {
          ...
          if (isPlayable) {
            ...
            const coinFlipVisible = await page.locator('[class*="coinFlipOverlay"]').isVisible().catch(() => false);
            if (coinFlipVisible) {
              console.log('Coin flip appeared, waiting for it to complete');
              // Click to skip coin flip animation if possible
              await page.locator('[class*="coinFlipOverlay"]').click().catch(() => {});
              // Wait for coin flip to complete
              await page.waitForSelector('[class*="coinFlipOverlay"]', { state: 'hidden', timeout: 10000 });
            }
      ...
      if (cardsPlayedThisRound > 0) {
        ...
        console.log(`Played ${cardsPlayedThisRound} cards this round, ending turn`);
        ...
        if (!isEnabled) {
          throw new Error('Cannot reduce technical debt and no cards are playable - game is stuck!');
        }
        ...
        throw new Error('Game logic error: Cards available but none played, and reduce TD not needed');
      }
      ...
      const victoryVisible = await page.getByText('Victory!').isVisible().catch(() => false);
      const gameOverVisible = await page.getByText('Game Over').isVisible().catch(() => false);
      ...
      if (finalCardCount === 0) {
        ...
        const drawingText = await page.getByText('drawing cards').isVisible().catch(() => false);
        if (!drawingText && !gameEnded) {
          throw new Error('No cards in hand but game has not ended and not drawing cards');
        }
      }
    }
    ...
    if (!gameEnded) {
      throw new Error(`Game did not end after ${maxRounds} rounds - possible infinite loop`);
    }
    
    // Verify we can see final results
    const finalScoreVisible = await page.getByText('Final Score').isVisible().catch(() => false);
    const roundsInfo = await page.getByText('Rounds:').isVisible().catch(() => false);
  ...
  test('should display correct statistics during gameplay', async ({ page }) => {
    await page.goto('/');
    await page.getByText('Start New Game').click();
    ...
    const initialRound = await page.locator('[class*="stat"]:has-text("Round") [class*="statValue"]').textContent();
    const initialTime = await page.locator('[class*="stat"]:has-text("Time") [class*="timeValue"]').textContent();
    const initialCardsPlayed = await page.locator('[class*="stat"]:has-text("Cards Played") [class*="statValue"]').textContent();
    const initialCardsRemaining = await page.locator('[class*="stat"]:has-text("Cards Remaining") [class*="statValue"]').textContent();
    ...
    if (cardCount > 0) {
      ...
      const newCardsPlayed = await page.locator('[class*="stat"]:has-text("Cards Played") [class*="statValue"]').textContent();
      expect(parseInt(newCardsPlayed!)).toBeGreaterThanOrEqual(parseInt(initialCardsPlayed!));
    }
  });
});
